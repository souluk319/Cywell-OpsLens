...
- `npm run verify:release-refresh` may be used before release-manager review to regenerate MVP, runtime/RAG, Lightspeed, certification readiness, catalog toolchain, image/provenance, security scan runner, OCP connectivity, OCP live reader smoke, Operator dry-run, release/external-runtime-review/install, checkpoint, roadmap, release bundle, and release action queue evidence in dependency order for the current Git HEAD.
