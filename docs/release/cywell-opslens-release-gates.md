...
- `npm run verify:release-refresh` may be used before release-manager review to regenerate MVP, runtime/RAG, Lightspeed, catalog toolchain, image/provenance, OCP connectivity, Operator dry-run, release/install, checkpoint, roadmap, and release bundle evidence in dependency order for the current Git HEAD.
