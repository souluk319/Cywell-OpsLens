...
- `npm run verify:catalog-toolchain` writes `test-results/cywell-opslens-catalog-toolchain-plan.json` plus Markdown, validates CSV/FBC/CatalogSource/Subscription/scorecard parity, records required CLI, registry.redhat.io auth, base-image manifest-read gaps, next action, owner, setup boundary, and keeps catalog validation/publish commands non-mutating or approval-gated.
...
- `npm run verify:lab-bootstrap` writes `test-results/cywell-opslens-lab-bootstrap-plan.json` plus Markdown before any CRC install rehearsal; it checks the portable image tar, manifest image references, bundle/catalog/runtime pullability gaps, CRC registry trap classifications, and optional lab-host CRC/GPU readiness while keeping registry login, image push/import, project creation, install apply, and OLSConfig patching approval-gated.
- `npm run verify:lab-image-map` writes preview-only JSON/Markdown/YAML for replacing owned Operator/API/dashboard/bundle/catalog image references with CRC registry image refs before an install rehearsal; it records local image presence, catalog image gaps, external runtime mirror review gaps, and approval-gated registry push commands without logging in, pushing, applying, patching, deleting, or scaling.
...
- `npm run verify:external-runtime-plan` passes before release publication and writes `test-results/cywell-opslens-external-runtime-images-plan.json`, keeping vLLM/Postgres/pgvector certification, vulnerability scan, SBOM, provenance, mirror digest, approval, risk, and rollback evidence separate from any registry mutation.
...
- `npm run evidence:security-review:promote -- --name operator|api|dashboard|bundle|catalog|vllm|pgvector --promote-reviewed --reviewer <reviewer> --review-ticket <ticket>` is the only scripted path from reviewed security-review drafts to final `*-security-review.json` evidence; it is local review-gated evidence only and performs no signing, pushing, mirroring, install, patch, apply, delete, or scale action.
...
- External runtime evidence templates live under `docs/release/evidence/external-runtime/*.example.json`; they define the required shape for real `vllm.json` and `pgvector.json` evidence but do not satisfy the release gate by themselves.
- `docs/release/evidence/external-runtime/README.md` is the reviewer intake contract for registry, security, release, and product owners; `npm run verify:external-runtime-plan` verifies the required roles, evidence states, and approval-gated mirror/sign boundaries.
- `npm run evidence:external-runtime:draft -- --all` may create ignored reviewer intake drafts for vLLM and Postgres/pgvector in one pass, and `npm run evidence:external-runtime:draft:digests` may read registry manifests to prefill available source digests, but draft status never satisfies release publication; final reviewed `vllm.json` and `pgvector.json` remain required.
- `npm run evidence:external-runtime:review-packet` writes a JSON plus Markdown packet for release/security reviewers with vLLM/Postgres/pgvector source digest state, final evidence gaps, reviewer requests, read-only refresh commands, and approval-gated mirror/sign commands that were not run.
- `npm run evidence:external-runtime:promote -- --name vllm|pgvector --promote-reviewed --reviewer <reviewer> --review-ticket <ticket>` is the only scripted path from draft intake to final external runtime evidence; it is a local review gate and performs no mirroring, signing, pushing, or cluster mutation.
...
- `npm run verify:release-refresh` may be used before release-manager review to regenerate MVP, runtime/RAG, Lightspeed, certification readiness, catalog toolchain, image/provenance, external runtime draft digest intake, security scan plan, OCP connectivity, OCP live reader smoke, Operator dry-run, release/external-runtime-review/install, checkpoint, roadmap, release bundle, and release action queue evidence in dependency order for the current Git HEAD. It runs `external-runtime-draft-digests` before `external-runtime-plan` so ignored reviewer intake drafts cannot remain on a stale head while the review packet is refreshed. It finishes with a final roadmap -> release bundle -> release action queue convergence pass so roadmap/action-queue freshness checks read current-head artifacts instead of the previous queue; inside this chain the action queue defers the release-refresh source because the refresh artifact is written after the final queue pass. It preserves existing security scan runner evidence by default; use `--security-scan-docker` when owned-image scan/SBOM evidence must be regenerated.
- `npm run verify:release-evidence-bundle` runs after the release, install, live handoff, OCP network handoff, external runtime review packet, checkpoint, roadmap, and catalog toolchain artifacts are refreshed; it writes `test-results/cywell-opslens-release-evidence-bundle.json` plus `test-results/cywell-opslens-release-evidence-bundle.md` as read-only release-manager packets with source artifacts, missing evidence, approvers, commands, risk, rollback, and mutation boundaries.
...
- External vLLM and Postgres/pgvector runtime images have a no-mutation certification/mirroring approval plan before release publication.
- vLLM and Postgres/pgvector real evidence files are created only after source digest, mirror digest, scan, SBOM, provenance, license/support, and approval artifacts exist.
