...
- `npm run verify:catalog-toolchain` writes `test-results/cywell-opslens-catalog-toolchain-plan.json`, validates CSV/FBC/CatalogSource/Subscription/scorecard parity, records required CLI, registry.redhat.io auth, and base-image manifest-read gaps, and keeps catalog validation/publish commands non-mutating or approval-gated.
