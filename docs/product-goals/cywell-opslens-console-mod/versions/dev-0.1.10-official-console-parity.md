...
| Kind-specific object detail summaries | Implemented | The shared drilldown now renders per-kind spec/status cards for Workload, Networking, Storage, Builds, Compute, RBAC, Operator, and Project resources, so Details no longer stop at generic metadata. |
...
| Builds lifecycle handoff | Implemented | Builds object drilldowns now expose resource-specific native handoff actions for Build logs/cancel review, BuildConfig start/edit/trigger review, and ImageStream tag import/history review. |
| Networking lifecycle handoff | Implemented | Networking object drilldowns now expose resource-specific native handoff actions for Route exposure/edit review, Service endpoint/selector review, Ingress edit review, and NetworkPolicy edit review. |
| Storage lifecycle handoff | Implemented | Storage object drilldowns now expose resource-specific native handoff actions for PVC expansion/workload impact review, PV reclaim/claim review, StorageClass default/provisioner review, and VolumeSnapshot restore/policy review. |
...
| Native object Details tab exposes per-kind OpenShift data | Static verifier checks the shared drilldown derives kind-specific spec/status summaries for Pod, Route, PVC, BuildConfig, Node, RBAC, Operator, and Project resources | `nativeDetailGroups`, `data-kind-summary`, `ocp-*-object-kind-summary-*` | Pass: 2026-06-20 local run, `verify:web-shell` 99 checks / 0 fail |
...
| Builds lifecycle actions remain native | Static verifier checks Builds passes resource-specific lifecycle handoff actions into the shared object drilldown | `buildLifecycleActions`, `start-build`, `cancel-build`, `tag-history` | Pass: 2026-06-20 local run, `verify:web-shell` 98 checks / 0 fail |
| Networking lifecycle actions remain native | Static verifier checks Networking passes resource-specific lifecycle handoff actions into the shared object drilldown | `networkingLifecycleActions`, `review-exposure`, `inspect-endpoints`, `edit-policy` | Pass: 2026-06-20 local run, `verify:web-shell` 98 checks / 0 fail |
| Storage lifecycle actions remain native | Static verifier checks Storage passes resource-specific lifecycle handoff actions into the shared object drilldown | `storageLifecycleActions`, `expand-pvc`, `reclaim-policy`, `snapshot-restore` | Pass: 2026-06-20 local run, `verify:web-shell` 98 checks / 0 fail |
| Contract prevents regression | Static verifier checks data-testid and helper function contracts | `npm run verify:web-shell` | Pass: 2026-06-20 local run, 99 checks / 0 fail |
...
3. Storage follow-up -> turn the native handoff actions into read-only impact previews: PVC expansion impact, PV reclaim risk, snapshot restore readiness, and StorageClass default/provisioner comparison.
4. Networking follow-up -> turn the native handoff actions into read-only route/service/ingress/policy topology overlays and approval-gated change plans.
5. Build follow-up -> turn the native handoff actions into read-only BuildConfig start/cancel/log previews and ImageStream tag-history timelines where the cluster exposes those routes.
