...
| Contract prevents regression | Static verifier checks data-testid and helper function contracts | `npm run verify:web-shell` | Pass: 2026-06-20 local run, 90 checks / 0 fail |
...
2. Compute/Admin follow-up -> Nodes, Machines, MachineSets, MachineConfigPools should get the same native dedicated surface instead of remaining only generic resource explorer surfaces.
3. User Management follow-up -> Users, Groups, ServiceAccounts, Roles, RoleBindings should get RBAC relationship views and native create/edit/delete handoff boundaries.
4. Storage follow-up -> add selected PVC/PV/StorageClass/Snapshot detail drill-down and native create/expand/restore deep links where the cluster exposes those console routes.
5. Networking follow-up -> add native create/edit/delete deep links and selected route/service/policy detail drill-down where the cluster exposes those console routes.
6. Build follow-up -> add native create/start/cancel/log deep links and selected Build detail drill-down where the cluster exposes those console routes.
7. Monitoring follow-up -> add deeper drill-down from Alerting/Dashboards/Metrics/Logs into exact native console URLs and selected object details where the cluster exposes the API.
