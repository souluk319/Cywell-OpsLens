...
| Resource API upstream sanitization | Raw upstream `OCP API ... returned 400` style messages are no longer displayed directly in the UI. | Backend and Resource Explorer map upstream non-success reads into named `ocp-upstream-read-failed` evidence, with raw response body withheld from customer-facing panels. |
| ConsolePlugin deep-link routing | The OpenShift Console navigation entry accepts `/opslens` and `/opslens/*` instead of only the exact root path. | `console.navigation/href` declares `startsWith: ["/opslens"]`; `console.page/route` uses `path: /opslens` with `exact: false` so direct/refresh paths are routed through the plugin launcher before handoff to the standalone asset. |
...
| Dashboard native signal board | Dashboard now uses live native-console overview fields for nodes, operators, workloads, network, builds, and alert pressure instead of fixed demo counts. | `opslens-native-signal-board`, `opslens-native-signal-source-label`, and per-signal cards render `consoleOverview.nodes`, `operators`, `workloads`, `networking`, `supplyChain`, and `monitoring` values with status meters. |
| Dashboard plus-alpha cockpit | Dashboard now shows the first customer-facing "why OpsLens is better than raw console" cockpit above the native map. | `opslens-plus-alpha-cockpit` merges Prometheus utilization coverage, workload drift, alert correlation, and OpsLens decision queue into four visual signal cards with source labels, meters, and a native-signal-to-OpsLens-overlay trace. |
...
- Continue replacing generic Resource Explorer screens with purpose-built native parity
  ...
  required.
- Continue upgrading the main dashboard beyond the first plus-alpha cockpit
  into deeper native-console metric visualization and drill-downs.
