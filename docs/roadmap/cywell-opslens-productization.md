...
| 5. Certification/GTM | Prepare Red Hat catalog and B2B packaging | `deploy/catalog/**`, `operators/cywell-opslens/**`, scorecard config, certification annotations, security/support/release docs, readiness verifier, Community Operator submission draft verifier, external runtime certification/mirroring plan, release publish approval plan | Static catalog/certification readiness, Community Operator submission draft parity, no-mirror external runtime plan, and no-push release publish plan pass first; Red Hat hosted/local certification pipeline, OperatorHub PR, and Partner Connect submission follow |
...
- Opt-in runtime RAG adapter contract: default `local` mode, explicit `hybrid`/`runtime` modes for vLLM embeddings plus Postgres/pgvector redacted snippet search, and local fallback with visible missing evidence.
...
| Runtime endpoint wiring can be checked safely | `npm run verify:runtime` verifies the Postgres/pgvector/vLLM service DNS, API route, and read-only readiness contract; `--live` additionally probes configured endpoints without mutation |
| Runtime RAG retrieval is evidence-gated | `npm run verify:runtime-rag` verifies `/ask`, `/mcp`, and incident analysis include `audit.runtimeRag`, default to local fallback, and only attempt vLLM/Postgres/pgvector when `CYWELL_OPSLENS_RAG_RUNTIME_MODE` is explicitly enabled; `npm run verify:runtime-rag:fixture` proves the hybrid success path against mock vLLM/Postgres/pgvector endpoints |
| Live cluster readiness can be checked safely | `npm run verify:lightspeed` reads the OLSConfig CRD and, when `CYWELL_OPSLENS_MCP_URL` is set, proves `tools/list` + `tools/call` without applying changes; `npm run verify:lightspeed:integration-handoff` captures the current live classification and the approval-gated registration rollback path |
...
2. Run `npm run verify:live-handoff` and `npm run verify:lightspeed:integration-handoff`, then execute the listed read-only commands from a machine that can reach the company OpenShift API: `npm run verify:operator:dry-run`, `npm run verify:lightspeed -- --mcp-url <cluster-or-local-mcp-url> --require-mcp`, and follow-up checkpoint refreshes.
...
4. Run `npm run verify:runtime-rag:fixture` before each runtime adapter change, then run `npm run verify:runtime -- --live` after Postgres/pgvector/vLLM services are reachable and enable `CYWELL_OPSLENS_RAG_RUNTIME_MODE=hybrid` for controlled live retrieval checks before replacing the local hash-vector index with production Postgres/pgvector/pgvector ingestion and live embedding jobs.
...
7. Run `npm run evidence:external-runtime:draft -- --all` to collect ignored reviewer intake packets, then run `npm run verify:external-runtime-plan` and `npm run verify:release-plan` after same-HEAD actual image build evidence is fresh; final readiness still requires reviewed vLLM/Postgres/pgvector digest, scan, SBOM, provenance, mirror, and approval evidence, catalog/certification placeholder replacement, `opm`, `operator-sdk bundle validate`, `operator-sdk scorecard`, image scanning, and Partner Connect submission once external tooling and images are available.
...
- Static manifests for API, dashboard, RAG policy ConfigMap, Postgres/pgvector vector store, vLLM runtime, ConsolePlugin, and ingress NetworkPolicies; OLSConfig registration stays outside the static app stack and only enters through the explicit `PatchOLSConfig` approval path or the standalone `deploy/lightspeed/` template.
...
- `npm run verify:external-runtime-plan` as the no-mirror vLLM/Postgres/pgvector certification, scan, SBOM, provenance, mirror digest, approval, risk, and rollback contract.
