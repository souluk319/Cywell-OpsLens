...
| Catalog toolchain | Catalog readiness records local CLI availability, registry.redhat.io auth presence plus read-only base-image manifest readability, CSV/FBC/CatalogSource/Subscription/scorecard parity, and separates read-only validation from setup and approval-gated publish commands. | `npm run verify:catalog-toolchain`, `test-results/cywell-opslens-catalog-toolchain-plan.json` |
