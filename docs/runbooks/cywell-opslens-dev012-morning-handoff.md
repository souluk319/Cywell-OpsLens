...
| Command | Result | Note |
| --- | --- | --- |
| `npm run verify:web-shell` | PASS | 0 fail, 50 checks after the release/action queue, external runtime review, security scan, catalog toolchain, lab readiness, certification, community submission, external runtime plan, owned provenance, and release publish label lanes |
| `npm run -w @kugnus/web build` | PASS | TypeScript, Vite, and ConsolePlugin bundle completed after the catalog/lab label lane |
...
| `npm run overnight:checkpoint` | PASS | latest clean checkpoint before Lane 76 passed 10/10 local gates on head `1121e9d`; evidence includes start/finish Git state and CRC demo readiness |
