...
async function expectConsoleFunctionEffect(
  ...
  test("AC-DASH-001 renders the dedicated OpsLens admin dashboard", async ({
    ...
    expect(body.installReadiness?.approvalPlan?.ticketPacket).toMatchObject({
    ...
    expect(body.installReadiness?.labHandoffPlan?.machineRolePlan).toMatchObject({
      workstation: {
        role: "repo-workstation",
        clusterMutationAllowed: false,
        registryMutationAllowed: false
    ...
    expect(
      body.installReadiness?.labHandoffPlan?.machineRolePlan?.transfer
        ?.missingTags
    ).toEqual(body.installReadiness?.labHandoffPlan?.imageTar?.missingTags);
    expect(
      body.installReadiness?.labHandoffPlan?.machineRolePlan?.labHost
        ?.approvalGatedCommandIds
    ).toEqual(
      expect.arrayContaining([
        "create-crc-project",
    ...
    if (
      ["registry-auth-required", "registry-permission-denied"].includes(
        vllmRegistryTicket?.classification ?? ""
      )
    ) {
      expect(vllmRegistryTicket?.registryAuthBoundary).toMatchObject({
        authRequired: true,
        humanCredentialInputRequired: true,
        credentialStoredByVerifier: false,
        pullSecretCreatedByVerifier: false,
        registryLoginExecutedByVerifier: false,
        firstHumanSetupAction:
          "registry-admin-authenticate-vllm-source-registry"
      });
    } else {
      expect(vllmRegistryTicket?.registryAuthBoundary).toMatchObject({
        credentialStoredByVerifier: false,
    ...
    const operatorSecurityReviewTicket = securityReviewTickets.find(
      (ticket) => ticket.imageName === "operator"
    );
    expect(operatorSecurityReviewTicket).toMatchObject({
      id: "security-reviewer-operator-security-review-ticket",
      owner: "security-reviewer",
      severity: "high",
      firstReadOnlyAction: {
        id: "security-review-operator",
        mutation: false,
        requiresExplicitApproval: false
      },
      approvalGatedAction: {
        id: "sign-owned-operator",
        mutation: true,
        requiresExplicitApproval: true
      },
      mutationBoundary: {
    ...
    expect(
      body.installReadiness?.releasePlan?.releaseManagerPacket
    ).toMatchObject({
      owner: "release-manager",
      markdownPath: expect.stringContaining(
        "cywell-opslens-release-publish-manager.md"
      ),
      exists: true,
      ticketId: "release-manager-release-publish-ticket",
      publishDecisionActionId: "release-manager-release-publish-decision",
      firstReadOnlyActionId: "run-release-preflight",
      humanSetupCommandIds: expect.arrayContaining(["login-release-registry"]),
      approvalGatedActionId: "approval-gated-push-operator",
      approvalGatedCommandIds: expect.arrayContaining(["push-operator"]),
      credentialStoredByVerifier: false,
      registryLoginExecutedByVerifier: false,
    ...
    expect(
      body.installReadiness?.refresh?.actionQueue?.readOnlyCommandCount ?? 0
    ).toBeGreaterThan(0);
    expect(
      body.installReadiness?.refresh?.actionQueue?.approvalGatedCommandCount ?? 0
    ).toBeGreaterThan(0);
    expect(
    ...
    expect(
      body.installReadiness?.refresh?.actionQueue?.ownerPackets?.every(
        (packet) => (packet.nextCommandCount ?? 0) > 0
      )
    ).toBe(true);
    expect(
      body.installReadiness?.refresh?.actionQueue?.ownerPackets?.every(
        (packet) => (packet.ticketPacketCount ?? 0) > 0
      )
    ).toBe(true);
    expect(
      body.installReadiness?.refresh?.actionQueue?.ownerPackets?.every(
        (packet) => (packet.readOnlyCommandIds?.length ?? 0) > 0
      )
    ).toBe(true);
    expect(body.installReadiness?.evidence?.join(" ")).toMatch(
    ...
    expect(completionGate.closeoutExecutionPlan?.length).toBe(
      completionGate.ownerCloseoutPackets?.length
    );
    expect(
      completionGate.closeoutExecutionPlan?.every(
        (row) =>
          row.owner &&
          row.status &&
          Array.isArray(row.gateIds) &&
          row.gateIds.length > 0 &&
          Array.isArray(row.ticketIds) &&
          row.ticketIds.length > 0 &&
          row.firstNextCommand &&
          row.firstReadOnlyCommandId &&
          row.firstReadOnlyCommandId !== "none" &&
          typeof row.readOnlyCommandCount === "number" &&
          row.readOnlyCommandCount > 0 &&
          typeof row.setupCommandCount === "number" &&
          typeof row.approvalGatedCommandCount === "number" &&
    ...
    expect(completionGate.claimPacket).toMatchObject({
      owner: "release-manager",
      status: expectedReadyToClaim100 ? "ready" : "needs-evidence",
      readyToClaim100: expectedReadyToClaim100,
      totalRequirements: roadmapPlan.completion?.totalRequirements,
      passedRequirements: roadmapPlan.completion?.passedRequirements,
      remainingRequirements: roadmapPlan.completion?.remainingRequirements,
    ...
    expect(completionGate.claimPacket?.markdownPath).toContain(
      "cywell-opslens-100-claim-packet.md"
    );
    expect(completionGate.claimPacket?.exists).toBe(true);
    expect(completionGate.claimPacket?.remainingGateIds).toEqual(
      completionGate.remainingTo100?.map((gate) => gate.gateId)
    );
    expect(completionGate.claimPacket?.ownerCloseoutPacketPaths).toEqual(
      completionGate.ownerCloseoutPackets?.map((packet) => packet.markdownPath)
    );
    expect(completionGate.claimPacket?.claimRequirementIds).toEqual(
      completionGate.claimRequirements?.map((requirement) => requirement.id)
    );
    expect(completionGate.claimPacket?.failedClaimRequirementIds).toEqual(
      completionGate.claimRequirements
    ...
    if (preClusterInstallGate.status !== "READY_FOR_CLUSTER_INSTALL") {
      ...
      expect(preClusterInstallGate.firstBlockedGate?.owner).toBeTruthy();
      expect(preClusterInstallGate.firstBlockedGate?.evidenceNeeded).toBeTruthy();
      expect(preClusterInstallGate.firstBlockedGate?.nextCommand).toBeTruthy();
      expect(preClusterInstallGate.firstBlockedGate?.readOnlyCommandId).toBe(
        "refresh-release-chain"
      );
      expect(preClusterInstallGate.firstBlockedGate?.strictCommandId).toBe(
        "pre-cluster-install-strict"
      );
    } else {
      expect(preClusterInstallGate.firstBlockedGate).toBeNull();
    }
    ...
    expect(
      body.installReadiness?.completionGate?.closeoutExecutionPlan?.map(
        (row) => row.owner
      )
    ).toEqual(completionGate.closeoutExecutionPlan?.map((row) => row.owner));
    expect(
      body.installReadiness?.completionGate?.closeoutExecutionPlan?.every(
        (row) =>
          row.owner &&
          row.firstNextCommand &&
          row.firstReadOnlyCommandId &&
    ...
    expect(body.installReadiness?.completionGate?.claimPacket).toMatchObject({
      owner: completionGate.claimPacket?.owner,
      status: completionGate.claimPacket?.status,
      markdownPath: completionGate.claimPacket?.markdownPath,
      exists: true,
      readyToClaim100: completionGate.claimPacket?.readyToClaim100,
      totalRequirements: completionGate.claimPacket?.totalRequirements,
      passedRequirements: completionGate.claimPacket?.passedRequirements,
      remainingRequirements: completionGate.claimPacket?.remainingRequirements,
      percentComplete: completionGate.claimPacket?.percentComplete,
      remainingGateIds: completionGate.claimPacket?.remainingGateIds,
      remainingExternalStateGateIds:
        completionGate.claimPacket?.remainingExternalStateGateIds,
      remainingLocalOnlyGateIds:
        completionGate.claimPacket?.remainingLocalOnlyGateIds,
      ownerCloseoutPacketPaths:
        completionGate.claimPacket?.ownerCloseoutPacketPaths,
    ...
    expect(certificationCriticalPath?.certificationToolingTicketPacket).toMatchObject({
      id: "release-manager-certification-tooling-ticket",
      severity: "high",
      firstReadOnlyAction: {
        id: "refresh-certification-evidence",
    ...
    if (clusterAdminOwnerPacket) {
      ...
      expect(clusterAdminOwnerPacket.firstInstallApprovalTicketPacket).toMatchObject({
        id: "cluster-admin-install-approval-ticket",
        firstReadOnlyAction: {
          id: "run-operator-server-dry-run",
          mutation: false
    ...
    expect(
      releaseManagerOwnerPacket?.firstCertificationToolingTicketPacket
    ).toMatchObject({
      id: "release-manager-certification-tooling-ticket",
      firstReadOnlyAction: {
        mutation: false
    ...
    if (
      ["registry-auth-required", "registry-permission-denied"].includes(
        vllmRegistryDigestAction?.externalRuntimeTicketPacket?.classification ?? ""
      )
    ) {
      expect(
        vllmRegistryDigestAction?.externalRuntimeTicketPacket?.registryAuthBoundary
      ).toMatchObject({
        authRequired: true,
        humanCredentialInputRequired: true,
        firstHumanSetupAction:
    ...
    expect(
      externalRuntimeLicenseCriticalPath?.externalRuntimeProductTicketPacket
    ).toMatchObject({
      id: "product-owner-vllm-external-runtime-license-ticket",
      severity: "high",
      classification: "license-support-review-required",
      firstReadOnlyAction: {
        id: "external-runtime-vllm-product-review",
        mutation: false,
        requiresExplicitApproval: false
      },
      approvalGatedAction: {
        id: "record-vllm-license-support-approval",
        mutation: false,
        requiresExplicitApproval: true
      },
      mutationBoundary: {
    ...
    expect(externalRuntimeLicenseCriticalPath?.readOnlyCommandIds).toEqual(
      expect.arrayContaining([
        "external-runtime-vllm-product-review",
        "refresh-external-runtime-drafts",
        "verify-external-runtime-plan"
      ])
    );
    ...
    expect(installApprovalCriticalPath?.installApprovalTicketPacket).toMatchObject({
      id: "cluster-admin-install-approval-ticket",
      firstReadOnlyAction: {
        id: "run-operator-server-dry-run",
        mutation: false
    ...
    expect(securityReviewAction?.securityReviewTicketPacket).toMatchObject({
      id: "security-reviewer-operator-security-review-ticket",
      severity: "high",
      imageName: "operator",
      firstReadOnlyAction: {
    ...
    expect(securityReviewCriticalPath?.securityReviewTicketPacket).toMatchObject({
      id: "security-reviewer-operator-security-review-ticket",
      severity: "high",
      firstReadOnlyAction: {
        mutation: false
    ...
    const securityReviewerOwnerPacket =
      body.installReadiness?.actionQueue?.ownerPackets?.find(
        (packet) => packet.owner === "security-reviewer"
      );
    expect(securityReviewerOwnerPacket?.firstSecurityReviewTicketPacket).toMatchObject({
      id: "security-reviewer-operator-security-review-ticket",
      firstReadOnlyAction: {
        id: "security-review-operator",
        mutation: false
    ...
    expect(body.installReadiness?.authRbacPlan?.ocContext).toMatchObject({
      currentContextSet: expect.any(Boolean),
      whoamiAvailable: expect.any(Boolean),
      showServerAvailable: expect.any(Boolean),
      kubeconfigEnvConfigured: expect.any(Boolean),
    ...
    expect(body.installReadiness?.authRbacPlan?.ticketPacket?.evidenceChecklist?.join(" ")).toMatch(
      /ocContext=|ocAuthenticationStatus=|ocServer=/
    );
    ...
    await expect(
      page.getByTestId("opslens-ocp-auth-rbac-plan-context")
    ...
    ).toContainText("cluster-admin-install-approval-ticket");
    await expect(
      page.getByTestId("opslens-install-approval-ticket")
    ).toContainText("first=run-operator-server-dry-run");
    await expect(
      page.getByTestId("opslens-install-approval-ticket")
    ).toContainText("approval=approval-gated-apply-operator-namespace");
    await expect(
      page.getByTestId("opslens-install-approval-ticket")
    ).toContainText("requiresApproval=true");
    await expect(
    ...
    await expect(
      page.getByTestId("opslens-lab-machine-role-plan")
    ).toContainText("transfer=portable-image-transfer");
    await expect(
      page.getByTestId("opslens-lab-machine-role-plan")
    ).toContainText("labHost=dedicated-crc-lab-host");
    await expect(
      page.getByTestId("opslens-lab-machine-role-plan")
    ).toContainText("companyOcpUsed=false");
    await expect(page.getByTestId("opslens-install-readiness")).toContainText(
      "Lab Bootstrap"
    );
    await expect(page.getByTestId("opslens-install-readiness")).toContainText(
    ...
    await expect(
      page.getByTestId("opslens-external-runtime-registry-tickets")
    ).toContainText(/authRequired=(true|false)/);
    await expect(
      page.getByTestId("opslens-external-runtime-registry-tickets")
    ).toContainText("credentialStored=false");
    await expect(
      page.getByTestId("opslens-external-runtime-registry-tickets")
    ).toContainText("registryLogin=false");
    await expect(
      page.getByTestId("opslens-external-runtime-registry-tickets")
    ).toContainText("requiresApproval=true");
    await expect(
    ...
    ).toContainText("security-reviewer-operator-security-review-ticket");
    await expect(
      page.getByTestId("opslens-security-review-tickets")
    ).toContainText("first=security-review-operator");
    await expect(
      page.getByTestId("opslens-security-review-tickets")
    ).toContainText("approval=sign-owned-operator");
    await expect(
      page.getByTestId("opslens-security-review-tickets")
    ).toContainText("requiresApproval=true");
    await expect(
    ...
    await expect(
      page.getByTestId("opslens-release-manager-publish-packet")
    ).toContainText("packet=cywell-opslens-release-publish-manager.md");
    await expect(
      page.getByTestId("opslens-release-manager-publish-packet")
    ).toContainText("exists=true");
    await expect(
      page.getByTestId("opslens-release-manager-publish-packet")
    ).toContainText("ticket=release-manager-release-publish-ticket");
    await expect(
      page.getByTestId("opslens-release-manager-publish-packet")
    ).toContainText("decision=release-manager-release-publish-decision");
    await expect(
      page.getByTestId("opslens-release-manager-publish-packet")
    ).toContainText("first=run-release-preflight");
    await expect(
    ...
    await expect(
      page.getByTestId("opslens-release-manager-publish-packet")
    ).toContainText("registryLoginExecuted=false");
    await expect(
      page.getByTestId("opslens-release-manager-publish-packet")
    ).toContainText("releasePublishExecuted=false");
    await expect(
      page.getByTestId("opslens-release-manager-publish-packet")
    ).toContainText("registryMutationAttempted=false");
    await expect(
    ...
    await expect(
      ...
      "productTicket=product-owner-vllm-external-runtime-license-ticket"
    );
    await expect(
      page.getByTestId("opslens-release-action-queue-critical-path")
    ).toContainText("productFirst=external-runtime-vllm-product-review");
    await expect(
      page.getByTestId("opslens-release-action-queue-critical-path")
    ...
    await expect(
      page.getByTestId("opslens-release-action-queue-critical-path")
    ).toContainText("certFirst=refresh-certification-evidence");
    await expect(
      page.getByTestId("opslens-release-action-queue-critical-path")
    ).toContainText(
      "securityTicket=security-reviewer-operator-security-review-ticket"
    );
    await expect(
      page.getByTestId("opslens-release-action-queue-critical-path")
    ...
    ).toContainText("cywell-opslens-100-claim-packet.md");
    await expect(
      page.getByTestId("opslens-completion-gate-claim-packet")
    ).toContainText(
      `status=${body.installReadiness?.completionGate?.claimPacket?.status}`
    );
    await expect(
      page.getByTestId("opslens-completion-gate-claim-packet")
    ).toContainText("exists=true");
    ...
    await expect(
      ...
      `readyToClaim100=${String(
        body.installReadiness?.completionGate?.claimPacket?.readyToClaim100
      )}`
    );
    await expect(
      page.getByTestId("opslens-completion-gate-claim-packet")
    ).toContainText("mutationBoundaryPassed=true");
    await expect(
    ...
    await expect(
      page.getByTestId("opslens-completion-closeout-execution-plan")
    ).toContainText(
      body.installReadiness?.completionGate?.closeoutExecutionPlan?.[0]
        ?.owner ?? "none"
    );
    await expect(
      page.getByTestId("opslens-completion-closeout-execution-plan")
    ...
    if (body.installReadiness?.preClusterInstallGate?.firstBlockedGate) {
      await expect(
        page.getByTestId("opslens-pre-cluster-install-gate-boundary")
      ).toContainText(
        `firstBlocked=${body.installReadiness.preClusterInstallGate.firstBlockedGate.id}:`
      );
      await expect(
        page.getByTestId("opslens-pre-cluster-install-gate-boundary")
      ).toContainText(
        `firstNext=${body.installReadiness.preClusterInstallGate.firstBlockedGate.nextCommand}`
      );
      await expect(
        page.getByTestId("opslens-pre-cluster-install-gate-boundary")
