...
`npm run evidence:external-runtime:draft -- --all` creates ignored `*.draft.json` reviewer packets for external runtime digest, scan, SBOM, provenance, license, and approval inputs. Use `--name vllm|qdrant` for a single packet or image-prefixed inputs such as `--vllm-source-digest` and `--qdrant-source-digest` for bulk intake. Drafts are surfaced by `npm run verify:external-runtime-plan`, but they never replace the final reviewed `docs/release/evidence/external-runtime/vllm.json` and `qdrant.json` release evidence files.
