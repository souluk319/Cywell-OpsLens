...
`npm run verify:lab-handoff` creates a non-mutating dedicated CRC lab handoff packet for the next server move. It checks Docker Linux engine readiness, local OpsLens API/dashboard/operator/bundle/catalog image tags, the portable CRC image tar, CRC target profile evidence, OCP API connectivity, Lightspeed readiness, OLSConfig patch preview, and install approval evidence, then writes `test-results/cywell-opslens-lab-server-handoff.json` plus Markdown with one next command. The portable tar is inspected for required `docker save` RepoTags so a missing bundle or catalog image is surfaced before OLM install rehearsal. It does not create projects, push images, apply manifests, patch OLSConfig, fetch Secrets, delete, or scale.
